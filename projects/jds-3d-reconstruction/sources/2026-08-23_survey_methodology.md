...
# 补充（2026-08-23 晚）：七次证伪后的修订

七个候选方向连续被证伪。以下为由此得到的、比原四通道更具体的教训。

## 6. 词汇假象出现了三次，这是规律不是意外

| 候选 | 窄词汇命中 | 扩展词汇后 | 真实表述 |
|---|---:|---:|---|
| 薄结构 | 3 篇 | 密集 | `high-frequency` |
| 变光照下重建 | 2 篇 | 25 篇 | `in-the-wild` / `appearance embedding` |
| 纹理缺失 | 2 篇 | 密集 | `planar prior` / `low-texture` |

**规律：命中数少于 5 篇时，几乎总是词汇问题，而不是空白。**

因此定为硬规则：**主题级命中 < 5 篇的结论一律不得直接采信**，必须先做同义词扩展并复核；复核前该格子记为「未知」而非「空白」。

## 7. 局限地图只能证否，不能证成

由「N 篇论文承认某局限、M 篇专门做」得出的缺口比 N/M，用途是**排除**（缺口比接近 1 表示供大于求，如稀疏视角 1.25×，确实不该碰），**不能用于选择**。

「纹理缺失」的缺口比为 ∞（33 篇承认、0 篇专攻），实际却有 PlanarGS (NeurIPS 2025)、Planar-Guided GS、2DGS-Room、GeoGS、DN-Splatter 等一批工作，且解法已收敛到平面/深度/法向先验。

原因：会议语料只覆盖有限几届，且专攻某问题的论文常以解法命名（`PlanarGS`）而非问题命名（`textureless`），标题与摘要中不出现问题词。

## 8. 别人的 future work 不是空位

MGSR 的 Limitations 写明「低光下过平滑，可考虑曝光补偿，作为未来工作」，看似留了口子。但实际检索发现该问题已被 Luminance-GS、PPISP、SEHDR 从不同角度覆盖。

**作者写下 future work 的时间点，与他人开始做的时间点，往往只差几个月。** 把 future work 当作空位，等于假设该领域无人阅读该论文。

## 9. 输入假设是划界的关键，不是方法

判断一篇工作是否占据某位置，问「它做了什么」会得出错误结论，应问「**它要求什么输入**」。

同为处理曝光/ISP 不一致：P2GS 要求多曝光、AERGS-SLAM 要求立体加在线、SEHDR 明确只要单曝光、PPISP 要求普通多视角。只有对齐输入假设，才能判断某设定是否真的空缺。

本项目曾据此认定「普通单曝光多视角的 ISP 变化」是空位，随后发现 PPISP 正是该设定——**该判断之所以出错，是因为只读了摘要的方法描述，未核对输入假设。**

## 10. 证伪成本在下降，这是唯一的正向信号

| 轮次 | 候选 | 证伪耗时 |
|---|---|---|
| 1 | 可编辑可重打光驾驶场景 | 两轮对话 |
| 2 | 观测受限行人重建 | 数小时 + 一次训练 |
| 3-6 | 密度控制/薄结构/几何有偏/训练可预测 | 约 20 分钟 |
| 7 | 纹理缺失、辐射-成像纠缠 | 约 1 小时 |

工具建成后，单个候选的证伪成本降到小时级。**这意味着策略应从「谨慎地选一个候选深入」转为「快速地证伪大量候选」**——前者在拥挤领域是高风险赌博，后者是可控搜索。
