...
产出仓库证据的分析与预处理脚本纳入 Git，位置为 `projects/<project-id>/tools/` 与 `.claude/skills/`。要求：

- 不得含硬编码绝对路径；执行器侧路径以环境变量或相对定位解决；
- 有对应的使用说明，并注明已验证与未验证的性质；
- 训练框架、数据集、模型权重与训练输出仍不入库，继续以逻辑 connection 引用。
